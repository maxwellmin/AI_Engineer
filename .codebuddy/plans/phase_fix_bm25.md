...
### 任务 2.1: 恢复 text_sparse 字段定义

**描述**: 取消注释 `DocumentCollectionSchema` 中的 `text_sparse` 字段

**验收标准**:
- [ ] 取消注释 `collection_schema.py` 第156-163行
- [ ] 根据 Milvus 2.5+ 特性调整字段定义
- [ ] 更新 `get_sparse_vector_field_names()` 测试用例
- [ ] 通过单元测试
...
### 任务 2.2: 更新 Collection Manager 测试

**描述**: 更新 `CollectionManager` 相关测试用例

**验收标准**:
- [ ] 更新测试用例包含 `text_sparse` 字段
- [ ] 验证 schema 创建成功
- [ ] 验证字段定义正确
...
### 任务 3.2: 更新 Index Manager 测试

**描述**: 更新 `IndexManager` 相关测试用例

**验收标准**:
- [ ] 更新测试用例验证 sparse index 创建
- [ ] 验证索引参数正确
- [ ] 通过单元测试
...
### 任务 4.1: 设计 BM25 Embedding 架构

**描述**: 设计 BM25 sparse vector 生成的架构方案

**验收标准**:
- [ ] 确定 BM25 提供者选择（Milvus 内置 vs 自定义）
- [ ] 设计 embedding engine 扩展接口
- [ ] 定义配置参数（k1, b 等）
- [ ] 记录设计决策
...
**验收标准**:
- [ ] 添加 BM25 provider 枚举（Milvus, Self）
- [ ] 添加 BM25 默认参数（k1, b）
- [ ] 添加 sparse embedding dimension（如需要）

**技术说明**:
```python
# apps/milvus_database_controller/constants.py
class BM25Provider(str, Enum):
    MILVUS = "milvus"  # Milvus built-in BM25 Function
    SELF = "self"      # External BM25 implementation

class SparseMetricType(str, Enum):
    IP = "IP"
    BM25 = "BM25"

# BM25 parameters (for Milvus built-in or custom)
...
### 任务 4.3: 实现 BM25 Client（如选择自定义方案）

**描述**: 如果选择自定义 BM25 方案，实现 BM25 embedding client

**验收标准**:
- [ ] 创建 `apps/embedding_engine/clients/bm25_client.py`
- [ ] 实现 `BM25Client` 类，继承 `BaseEmbeddingClient`
- [ ] 支持中文分词（jieba）
- [ ] 输出 Milvus 兼容的 sparse vector 格式
- [ ] 通过单元测试

**技术说明**:
```python
# apps/embedding_engine/clients/bm25_client.py
from rank_bm25 import BM25Okapi
import jieba

class BM25Client(BaseEmbeddingClient):
    def embed_batch(self, texts: list[str]) -> list[dict[int, float]]:
        """Generate sparse vectors for texts.
        
        Returns:
            List of sparse vectors as {token_id: score} dict
        """
        # 1. Tokenize texts
        ...
        # 3. Convert to sparse vector format
        # ...
```
...
### 任务 4.4: 更新 Embedding Service

**描述**: 扩展 `EmbeddingService` 支持 sparse embedding

**验收标准**:
- [ ] 添加 `generate_sparse_embedding()` 方法
- [ ] 支持配置选择 BM25 provider
- [ ] 处理 dense + sparse embedding 组合
- [ ] 通过单元测试

**技术说明**:
```python
# apps/embedding_engine/services/embedding_service.py
class EmbeddingService:
    def generate_embeddings(
        ...
        dense = self.client.embed_batch(texts)
        
        if include_sparse:
            sparse = self.sparse_client.embed_batch(texts)
...
### 任务 5.1: 实现 BM25 Search 方法

**描述**: 完善 `SearchManager.bm25_search()` 实现

**验收标准**:
- [ ] 实现 sparse vector 搜索逻辑
- [ ] 支持配置 BM25 参数
- [ ] 返回正确的搜索结果格式
- [ ] 通过单元测试
...
**验收标准**:
- [ ] 实现多向量字段搜索（summary_dense + text_dense）
- [ ] 实现 dense + sparse 混合检索
- [ ] 实现 RRF 或 weighted fusion 算法
- [ ] 支持配置融合策略
- [ ] 通过单元测试
...
### 任务 5.3: 更新 Search API View

**描述**: 更新 `search_views.py` 支持 BM25 和混合检索

**验收标准**:
- [ ] 验证 BM25 search endpoint 正常工作
- [ ] 验证 hybrid search endpoint 正常工作
- [ ] 更新 API 文档（drf-spectacular）
- [ ] 通过集成测试
...
### 任务 6.1: Collection 重建脚本（简化版）

**描述**: 创建脚本重建 collection 以启用 BM25 Function

**验收标准**:
- [ ] 创建 `rebuild_collection.py` 脚本
- [ ] 删除现有 collection
- [ ] 使用新 schema（含 BM25 Function）创建 collection
- [ ] 创建索引
...
**验收标准**:
- [ ] 插入测试文档（仅 text 字段）
- [ ] 验证 text_sparse 字段自动生成
- [ ] 验证 sparse vector 格式正确
- [ ] 验证 BM25 搜索返回正确结果
...
**验收标准**:
- [ ] `test_collection_manager.py` - schema 创建测试
- [ ] `test_index_manager.py` - sparse index 测试
- [ ] `test_search_manager.py` - BM25 和混合检索测试
- [ ] `test_embedding_service.py` - sparse embedding 测试
- [ ] 所有测试通过
...
**验收标准**:
- [ ] 上传新文档并执行 pipeline
- [ ] 验证 sparse vector 正确生成和存储
- [ ] 验证 BM25 搜索返回正确结果
- [ ] 验证混合检索融合正确
- [ ] 更新 `apps/document_pipeline_manager/docs/manual_test.md`
...
- [ ] 生成 BM25 检索测试用例
- [ ] 生成混合检索测试用例
- [ ] 测试用例包含在 `apps/milvus_database_controller/docs/manual_test.md`
- [ ] 测试用例包含预期结果和错误场景
...
**验收标准**:
- [ ] 更新 `docs/architecture.md` 说明 BM25 实现
- [ ] 更新 `docs/BM25_procedure.md` 添加实现细节
- [ ] 更新 `apps/milvus_database_controller/docs/manual_test.md`
- [ ] 更新 `dev_utils/README.md` 说明 Milvus 版本要求
