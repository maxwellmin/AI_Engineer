...
422 Unprocessable Entity  — 语义无效（JSON 有效但数据不正确）
...
# 坏：验证错误返回 500
# 好：返回 400 或 422 并附带字段级详情

# 坏：创建资源返回 200
# 好：返回 201 并附带 Location 头
...
**优点：** 易于实现，支持"跳转到第 N 页"
**缺点：** 大偏移量时性能差（OFFSET 100000），并发插入时不一致
